...
from services.auth import current_plan, current_user, user_has_plan_access, user_plan_memberships
...
def recipe_edit_view(id):
    ...
    recipe. Nur erreichbar, wenn das Rezept für den aktuell aktiven Plan
    SICHTBAR ist (Eigentümer ODER per RecipePlanLink eingebunden, siehe
    ...
    (recipe.owner_plan_id), nicht aus dem gerade aktiven Plan: Recipe.
    ...
    einbinden"-Steuerelement (siehe templates/recipe_form.html)."""
    plan = current_plan()
    recipe = visible_recipes_query(plan.id).filter(Recipe.id == id).first()
    if recipe is None:
        abort(404)
    ...
    # bewusst der des AKTIVEN Plans (nicht des Eigentümers) - wer ein nur
    # eingebundenes Rezept bearbeitet, soll seine EIGENEN Gleichsetzungen/
    # Anzeige-Einheiten sehen, siehe services/planning.py: jsonify_recipe-
    # Docstring für denselben Grundsatz auf der Plan-Seite.
    selected_presets, custom_range = describe_recipe_seasons(recipe)
    display_units = get_display_units(plan.id)
    ingredient_display = {}
    for ing in recipe.ingredients:
        display_amount, display_unit = convert_for_display(ing.amount, ing.unit, display_units)
        ingredient_display[ing.id] = (display_amount, display_unit, normalize_ingredient_name(plan.id, ing.name))

    linked_plan_ids = {link.plan_id for link in recipe.plan_links} | {recipe.owner_plan_id}
    linkable_plans = [
        m.plan for m in user_plan_memberships(current_user())
...
def recipe_edit_list_view():
    """Zeigt die reine Übersichtsliste aller für den aktiven Plan
    ...
    return render_template(
        'recipe_edit_list.html', recipes=recipes, recipe_labels=recipe_labels, own_plan_id=plan.id,
    )
...
def edit_recipe(id):
    ...
    if recipe.nutrition_override:
        ...
        computed = compute_recipe_nutrition(plan.id, normalized_ingredients, recipe.servings)
...
def delete_recipe(id):
    ...
    if recipe.owner_plan_id != plan.id:
        abort(403)
...
def link_recipe_to_plan(id, target_plan_id):
    ...
    aktiven Plan sichtbares Rezept ZUSÄTZLICH an target_plan_id ein (siehe
    models.py: RecipePlanLink) - eine echte Verknüpfung, keine Kopie.
    Erfordert, dass der eingeloggte Nutzer tatsächlich Mitglied von
    target_plan_id ist (sonst könnte er fremde Pläne mit Rezepten
    "zuspammen", auf die er selbst gar keinen Zugriff hat)."""
    plan = current_plan()
    recipe = visible_recipes_query(plan.id).filter(Recipe.id == id).first()
    if recipe is None:
        abort(404)
    if not user_has_plan_access(current_user(), target_plan_id):
        abort(403)
    if target_plan_id != recipe.owner_plan_id and not RecipePlanLink.query.filter_by(
        ...
        db.session.commit()
    return redirect(url_for('recipes.recipe_edit_view', id=id))
