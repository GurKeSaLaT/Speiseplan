...
def test_index_has_no_dice_or_create_buttons(app, client, make_recipe):
    ...
    friday = friday_of(_date.today())
    with app.app_context():
        db.session.add(PlanDay(plan_id=client.plan_id, date=friday, main_recipe_id=recipe_id, servings=2))
        db.session.commit()
...
def test_summary_open_recipe_switches_plan_and_redirects_with_open_day(app, client, make_user, make_recipe):
    ...
    resp = client.post("/plan/summary/open", data={"plan_id": other_plan_id, "date": friday.isoformat()})
    assert resp.status_code == 302
    assert resp.headers["Location"] == f"/plan/{friday.isoformat()}?plan_id={other_plan_id}&open_day={friday.isoformat()}"
...
def test_week_view_redirects_non_friday_to_friday(client):
    wednesday = date(2026, 6, 17)
    resp = client.get(f"/plan/{wednesday.isoformat()}")
    assert resp.status_code == 302
    assert "2026-06-12" in resp.headers["Location"]
...
def test_week_view_non_friday_redirect_preserves_plan_id(client):
    ...
    assert resp.status_code == 302
    assert resp.headers["Location"] == f"/plan/2026-06-12?plan_id={client.plan_id}"
...
def test_week_view_prev_next_and_recreate_links_carry_plan_id(app, client, make_recipe):
    ...
    html = resp.get_data(as_text=True)
    assert f"/plan/2026-06-05?plan_id={client.plan_id}" in html
    assert f"/plan/2026-06-19?plan_id={client.plan_id}" in html
    assert f"/plan/2026-06-12/create?plan_id={client.plan_id}" in html
...
def test_week_view_plan_data_reflects_excluded_and_servings(client, app):
    ...
    friday = date(2026, 6, 12)
    with app.app_context():
        db.session.add(PlanDay(plan_id=client.plan_id, date=friday, excluded=True, servings=4))
        db.session.commit()
...
def test_week_view_plan_data_reflects_cooked_main(client, app, make_recipe):
    ...
    with app.app_context():
        db.session.add(PlanDay(plan_id=client.plan_id, date=friday, main_recipe_id=recipe_id, cooked=True))
        db.session.add(PlanDay(plan_id=client.plan_id, date=friday + timedelta(days=1), cooked=False))
        db.session.commit()
...
def test_week_view_extra_items_use_display_unit(client, app):
    ...
    with app.app_context():
        ...
        db.session.add(ExtraShoppingItem(plan_id=client.plan_id, week_start=friday, name="Mehl", amount=2000, unit="g"))
        db.session.commit()
...
def test_week_create_view_uses_explicit_plan_id(app, client, make_user, make_recipe):
    ...
    assert f"/plan/2026-06-12?plan_id={other_plan_id}" in resp.get_data(as_text=True)
...
def test_week_generate_with_explicit_plan_id_writes_to_that_plan(app, client, make_user, make_recipe, make_category):
    ...
    assert resp.status_code == 302
    assert resp.headers["Location"] == f"/plan/2026-06-12?plan_id={other_plan_id}"

    with app.app_context():
        own_day = PlanDay.query.filter_by(plan_id=client.plan_id, date=date(2026, 6, 12)).first()
        other_day = PlanDay.query.filter_by(plan_id=other_plan_id, date=date(2026, 6, 12)).first()
...
def test_week_generate_fixed_assignment_is_respected(client, app, make_recipe):
    ...
    with app.app_context():
        friday_row = PlanDay.query.filter_by(date=date(2026, 6, 12)).first()
        assert friday_row.main_recipe_id == fixed_id
...
def test_week_generate_fills_all_seven_days(client, app, make_recipe):
    ...
    with app.app_context():
        rows = PlanDay.query.filter(
            PlanDay.date.in_([date(2026, 6, 12) + timedelta(days=i) for i in range(7)])
        ).all()
        assert len(rows) == 7
...
def test_week_generate_excluded_day_has_no_main_recipe(client, app, make_recipe):
    ...
    with app.app_context():
        friday_row = PlanDay.query.filter_by(date=date(2026, 6, 12)).first()
        assert friday_row.excluded is True
        assert friday_row.main_recipe_id is None
...
def test_week_generate_assigns_fixed_side_dishes(client, app, make_recipe):
    ...
    with app.app_context():
        friday_row = PlanDay.query.filter_by(date=date(2026, 6, 12)).first()
        side_recipe_ids = {s.recipe_id for s in PlanDaySide.query.filter_by(plan_day_id=friday_row.id).all()}
...
def test_other_plan_meals_shows_dish_from_second_plan_same_day(app, client, make_recipe, make_user):
    ...
    with app.app_context():
        db.session.add(PlanDay(plan_id=client.plan_id, date=friday, main_recipe_id=own_recipe_id, servings=2))
        db.session.add(PlanDay(plan_id=other_plan_id, date=friday, main_recipe_id=other_recipe_id, servings=2))
        db.session.add(PlanMembership(plan_id=other_plan_id, user_id=client.user_id, is_starred=False))
        db.session.commit()
...
def test_other_plan_meals_respects_per_membership_overview_flag(app, client, make_recipe, make_user, login_as):
    ...
    with app.app_context():
        db.session.add(PlanDay(plan_id=shared_plan_id, date=friday, main_recipe_id=shared_recipe_id, servings=2))
